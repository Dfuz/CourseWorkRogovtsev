...
int main(int argc, char *argv[]){
    ...
    if (argc == 1) // если нет аргументов
    {
        ...
        while (--argc > 0)
        {
            ...
            if (error == 1) { // проверка на открытие файла
                fprintf (stderr, "\nНевозможно открыть файл №%d для чтения.\n", i);
                exit(2);
            }
            if (error == 2) { // проверка на пустоту файла
                fprintf (stderr, "\nФайл №%d пуст.\n", i);
                exit(3);
            }
            ...
            if (error == 1) { // проверка на ошибки обработки файла
        ...
        else if ( (c == '.')||(c == '!')||(c == '?')||(c == '…') )
        {
            ...
            if (  c == '.' )
                while ( ((c == '.')||(c == '!')||(c == '?')||(c == '…'))&&(c != EOF) ){
                    putc( c, ofp );
                    c = getc( ifp );
                    if ( ferror(ifp) ) {fclose(ofp); return 1;} // если произошла ошибка
                }
            if (c != EOF){
                fprintf(ofp, "\n\r");
                if ( (c != '\n')&&(c != ' ') ) putc( c, ofp );
